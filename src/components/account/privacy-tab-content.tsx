...
/**
 * Re-exports the existing PrivacySettingsPage as a default export for React.lazy().
 * The component is imported from the original datenschutz page to avoid duplication.
 */
...
export default function PrivacyTabContent() {
  ...
  const [trainerConnections, setTrainerConnections] = React.useState<
    TrainerConnection[]
  >([]);
  ...
  async function updateConsent(type: string, granted: boolean) {
    ...
    try {
      ...
      toast.success(
        granted ? t("consentGrantedSuccess") : t("consentRevoked")
      );
    } catch {
      toast.error(t("errorGeneric"));
    } finally {
              ...
              {getConsent("terms_privacy") && (
                <p className="text-caption text-muted-foreground">
                  ...
                  {t("consentPolicyVersion", {
                    version: getConsent("terms_privacy")!.policy_version,
                  })}
                </p>
              )}
            </div>
            ...
            {trainerConnections.map((conn, idx) => (
              ...
              >
                ...
                <div className="flex flex-wrap gap-2">
                  <Badge
                    variant={conn.can_see_body_data ? "default" : "outline"}
                  >
                  ...
                  <Badge
                    variant={conn.can_see_nutrition ? "default" : "outline"}
                  >
                  ...
                  <Badge
                    variant={conn.can_see_calendar ? "default" : "outline"}
                  >
        ...
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button
              variant="destructive"
              onClick={() =>
                revokeType && updateConsent(revokeType, false)
              }
              disabled={isRevoking}
            >
              {isRevoking && (
